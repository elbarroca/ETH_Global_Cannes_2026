...
| `NEON-WALLET-AUTHORITY-MIGRATE-20260726-01` | `CONSUMED` | Project owner | Implementation SHA `86150aebb9c3321a2e4fc57186dbcc8e08a3d9df`; existing Neon endpoint `ep-hidden-sound-ay5ce4gb`, database `neondb`; read-only `prisma migrate status`, then one `prisma migrate deploy` through the matching non-pooled `DIRECT_URL`, then one status/schema readback; proceed only when Prisma finds exactly 22 committed migrations and the pending set is exactly `20260725230000_mcp_hire_claim_replay` and `20260725240000_wallet_authority_publication`; no reset, development migration, seed, provisioning, credential change, push, deploy, transaction, or repeat | 2026-07-26 02:09 WEST; narrowed 02:12 WEST | Consumed once at 02:12-02:13 WEST. Initial five-migration condition did not match and caused zero writes. Narrowed preflight found exactly the two authorized pending migrations; both applied successfully. Post-status reports schema current at 22/22. Direct and pooled readbacks confirm both wallet tables, publication and receipt authority columns, nullable legacy ENS receipt authority, hireability function, and wallet/decision integrity triggers. Secret omitted. |
| `CIRCLE-WALLET-READONLY-DIAGNOSTIC-20260726-01` | `CONSUMED` | Project owner | Implementation SHA `7d77e6bf39f5f96be860a3d48cb4848f540de292`; Circle TEST developer-controlled wallet account configured in `.env.local`; configured wallet-set SHA-256 fingerprint `b03ecdfdcfe8860d4db581ed5b846d365a5c11a44c6257c26980717a92328c15`; agent `4e83f2cf-d549-4774-a3c9-911af891d413`; version `b03f0f90-9cfe-434a-a129-57c58d8d5c8d`; exactly one `getWalletSet` for the configured set and one `listWallets` filtered by that set, exact agent refId, and `UNI-SEPOLIA`; no create, update, derive, fund, faucet, sign, transaction, secret output, repeat, push, or deploy | 2026-07-26 02:34 WEST | Consumed once at 02:35 WEST. Wallet-set read succeeded with exact ID match and `DEVELOPER` custody. Exact agent-refId/`UNI-SEPOLIA` list succeeded with zero wallets. Credentials and wallet-set binding are valid; the refusal is in wallet creation. No mutation occurred and secrets were omitted. |
