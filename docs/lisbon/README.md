...
Start one persistent Codex task at this repository root, then paste [`prompts/C0-A4-CONTINUATION.md`](prompts/C0-A4-CONTINUATION.md).

## Run first

1. Open one persistent task on `Eth_global_lisbon_` and paste the A4 continuation C0 only.
...
Accepted A0-A3 engineering remains on `developer`; A4-A7 continuation work stays on `Eth_global_lisbon_`. No prompt may silently broaden authority granted by `EXTERNAL-EFFECTS.md`.
