...
| A2 authenticated commerce kernel | `PASS_TO_AUDIT; LOCAL_ONLY; A3_NOT_CONFIGURED` | Independent audit of one clean A2 SHA must confirm canonical SIWE/session boundaries, immutable server-owned versions, 20-way idempotency, deterministic worker recovery, exclusive settlement/refund, protected boot, and exact evidence in [`evidence/A2-AUTHENTICATED-KERNEL.md`](evidence/A2-AUTHENTICATED-KERNEL.md). Live adapters and every release claim remain blocked. |
...
| 0G Keep qualification | `research_only_not_promotable; P0_GO_PATH_FOUND; LIVE_EFFECT_BLOCKED` | Integrate the pinned official Go proof verifier, make Compute binding fatal, pass A3 fixture/integration, obtain exact live authority and identifiers, then pass A7 release validation. |
