...
| `A1-FOUNDATION-001` | Foundation | `PASS_LOCAL_ONLY` | One local tree rooted at `8c07b80a515405947b5994c6540f9038e7f5058f` passes the lockfile install, Prisma validate/generate, lint, typecheck, unit, disposable-database integration, and production-build gate. Exact scope, commands, synthetic replay boundary, failures repaired, and remaining blocks: [`evidence/A1-DETERMINISTIC-FOUNDATION.md`](evidence/A1-DETERMINISTIC-FOUNDATION.md). |
...
`A0_LOCAL` opened Goal A1. A1's `PASS_LOCAL_ONLY` opens A2 and authorizes offline A3 after its sequential code prerequisites pass. Rights/license/team/owner gaps, required access and caps, event-window evidence, live P0 proof, and independent release audit remain `RELEASE_BLOCKED`, claim-blocked, or `LIVE_EFFECT_BLOCKED`. Pre-H0 work remains disclosed prior/pre-window work and cannot be classified as Lisbon-window evidence.
