...
| `A2-AUTHENTICATED-KERNEL-REMEDIATION-003` | A2 remediation G1 | `PASS_TO_REAUDIT; LOCAL_ONLY` | Owner+epoch+claim-version+expiry fencing now guards every worker mutation; expired-version reconciliation is separate; heartbeat loss suppresses terminal persistence; retry cancellation terminalizes its effect; SIWE action separation, fresh authenticate user resolution, and malformed-cookie refusal pass. Exact packet: [`evidence/A2-AUTHENTICATED-KERNEL.md`](evidence/A2-AUTHENTICATED-KERNEL.md). |
...
A2 remediation G1 is `PASS_TO_REAUDIT; LOCAL_ONLY`. Its PostgreSQL clusters, EOA signatures, concurrency, lease takeover, kill/restart, and adapter results are deterministic local fixtures. Protected startup performs no sponsor call, and production adapter execution terminates `A3_NOT_CONFIGURED`. No live browser signature, external effect, shared database, public identifier, or sponsor proof was obtained. Details and exact counts are in [`evidence/A2-AUTHENTICATED-KERNEL.md`](evidence/A2-AUTHENTICATED-KERNEL.md).
