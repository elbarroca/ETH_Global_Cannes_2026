...
A4 is `PASS_FIXTURE; PASS_INTEGRATION; PASS_TO_AUDIT; LOCAL_ONLY`. Stable ENS resolution is implemented behind exact `viem@2.47.6`, but every exercised resolver response was a deterministic local fixture and every PostgreSQL cluster was disposable loopback state. Fresh authority gates each remaining strict A3 operation and delivery; the database rejects receipt bypass independently. The six focused A4 tests and combined 23-test integration lane cover malformed/stale/missing/wrong parent, agent, chain, version, registry, resolver, owner/delegate, manifest, service, replay, outage, timeout, transfer, restart, both resume stages, both authority-phase lease takeovers, 20 duplicates, and DB immutability/bounds. No live ENS read/write or public identifier exists. Exact details are in [`evidence/A4-ENS-AUTHORITY.md`](evidence/A4-ENS-AUTHORITY.md).

## Downstream boundary

`A0_LOCAL` opened A1; A1 opened A2; accepted A2 remediation opened offline A3; accepted A3 remediation opened local A4. A4 now returns `PASS_TO_AUDIT` on the containing SHA; A5 remains closed until an independent pinned-SHA A4 audit passes. Rights/license/team/owner gaps, required access and caps, event-window evidence, live 0G/ENS proof, inherited README claim drift, deployment, and independent release audit remain `RELEASE_BLOCKED`, claim-blocked, or `LIVE_EFFECT_BLOCKED`. Pre-H0 work remains disclosed prior/pre-window work and cannot be classified as Lisbon-window evidence.
