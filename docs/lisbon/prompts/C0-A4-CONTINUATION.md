# C0 - AlphaDawg Lisbon A4-to-Release Continuation

Paste this `/goal` once into one persistent Codex task. Do not launch the executor or auditor manually.
...
- Repository: AlphaDawg Lisbon Continuity.
...
9. `docs/lisbon/evidence/A2-AUTHENTICATED-KERNEL.md`
10. `docs/lisbon/evidence/A3-STRICT-0G.md`
11. only the current context files needed by A4-A7

The archive is provenance only. It cannot authorize work or override current controls.

BOOT AND RECONCILIATION

- Resolve the repository with `git rev-parse --show-toplevel`.
...
- Reproducible npm/Prisma/PostgreSQL foundation, CI, environment validation, migration replay, build, redaction, resilience, and secret checks.
- Canonical SIWE challenge/session authentication and DB-only onboarding.
...
- Pasting this goal authorizes safe local files, tests, loopback services, disposable databases, and atomic commits on `Eth_global_lisbon_` for A4-A7.
- The already-authorized branch publication does not authorize a PR, merge, deployment, managed migration, sponsor/paid API call, signature, transaction, funding, upload, form, spend, mainnet value, or public claim.
- An external effect opens only through an exact current `AUTHORIZED` row in `docs/lisbon/EXTERNAL-EFFECTS.md` binding effect, SHA, provider/network, scope, cap, deadline, and evidence. Credentials are setup, not authority.
...
- Before any edit, atomically acquire `$(git rev-parse --git-common-dir)/alphadawg-lisbon-writer.lock`.
- Mirror the same unique token, branch, start SHA, expiry, task, and allowed paths in `docs/lisbon/ACTIVE-WRITER.md`.
- Lock failure, stale/unknown ownership, or token mismatch is `BLOCKED_WRITER_LOCK`; never self-clear another writer.
- A writer exits only after proportional checks, evidence updates, one atomic commit, clean status, mirror closure, and release of its own token.
...
Run the relevant focused checks plus, before accepting a sprint:

- `npm ci --legacy-peer-deps`
- `npx prisma validate`
- `npx prisma generate`
- both empty and inherited synthetic migration lanes
...
- `npm run build`
- `git diff --check`, exact changed-path review, and secret-pattern review

Add real A4/A5 checks; missing, skipped, placeholder, or always-pass commands are blockers. Keep inherited warnings/advisories explicit and separate from new failures.

CLAIMS AND EVIDENCE

- Evidence ladder: `NOT_RUN -> PASS_FIXTURE -> PASS_INTEGRATION -> PASS_LIVE -> PASS_RELEASE`.
- Preserve `research_only_not_promotable` until exact promotion evidence exists.
- A fixture, mock, installed SDK, HTTP 200, request ID, inherited receipt, or UI badge is never live proof.
- Pre-event work remains disclosed prior/pre-window work and cannot be relabeled Lisbon-new without evidence.
- A changed implementation SHA, expired official rule, failed later audit, or mismatched deployment makes affected evidence stale.
- Update `EVIDENCE.md`, `CLAIM-MATRIX.md`, `TRACK-MATRIX.md`, `EXTERNAL-EFFECTS.md`, `CHANGELOG-LISBON.md`, and exact evidence packets only with observed facts.

RECOVERY AND STOP RULES

- Retry a transient read-only/tool failure once. Deterministic failures require root-cause repair, not blind rerun.
...
When an external gate is reached, request only the minimum missing authority and report:

- exact effect;
- target SHA;
- provider/network/project/environment;
- non-secret credential names and where they are securely configured;
- request/write/deploy scope;
- atomic spend/gas cap and deadline;
- expected public identifiers/evidence;
- rollback/reconciliation plan;
- what safe work is already complete.
...
Declare `RELEASE_VALIDATED` only when the full release contract passes on one unchanged SHA. Otherwise return the exact non-terminal checkpoint and next owner wake condition without promoting claims.
```
