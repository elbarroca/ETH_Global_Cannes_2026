    ...
    <main className="mx-auto max-w-[90rem] space-y-5 px-4 py-6 sm:px-6 lg:py-8">
      <section className="grid gap-4 border-b border-void-800 pb-5 lg:grid-cols-[minmax(0,2fr)_minmax(10rem,0.55fr)_minmax(11rem,0.65fr)_auto] lg:items-end">
        ...
        <div>
          ...
          <p className="mt-2 font-mono text-sm font-semibold text-void-200">{formatUsdcAtomic(goal.policy.perRunCapAtomic)} / run</p>
          <p className="mt-1 font-mono text-xs text-void-500">{goal.policy.dailyCapAtomic ? `${formatUsdcAtomic(goal.policy.dailyCapAtomic)} / day` : "No daily cap"}</p>
        </div>
    ...
    <div className="grid gap-8 border-y border-void-800 py-6 lg:grid-cols-[minmax(0,7fr)_minmax(18rem,5fr)]">
      ...
      <aside className="self-start border-y border-void-800 py-5 lg:sticky lg:top-24" aria-label="Live goal summary">
        ...
        <dl className="mt-4 divide-y divide-void-800 text-sm">
          ...
          <SummaryRow label="Per run" value={policy.perRunCapAtomic ? formatUsdcAtomic(policy.perRunCapAtomic) : "Unset"} mono />
          <SummaryRow label="Daily" value={policy.dailyCapAtomic ? formatUsdcAtomic(policy.dailyCapAtomic) : "Not applicable"} mono />
...
function SelectedAgents({ run }: { run: GoalRunSnapshot | null | undefined }) { return <section><h2 className="font-semibold text-void-100">Matched agents</h2>{!run?.jobs.length ? <p className="mt-3 text-sm text-void-500">No external canonical version matched this run.</p> : <ul className="mt-3 divide-y divide-void-800">{run.jobs.map((job) => <li key={`${job.role}-${job.selectionRank}`} className="py-3"><div className="flex flex-wrap items-center justify-between gap-2"><span className="break-all font-mono text-sm text-dawg-300">{job.fullSubname}</span><span className="text-xs text-void-500">{job.role}</span></div><p className="mt-2 font-mono text-xs text-void-400">Version {job.agentVersionId.slice(0, 8)}, {formatUsdcAtomic(job.priceAtomic)}</p><details><summary className="min-h-11 cursor-pointer py-3 text-xs text-void-400">Version, capabilities, and proof identifiers</summary><dl className="space-y-2 break-all font-mono text-xs text-void-400"><div><dt>Version ID</dt><dd>{job.agentVersionId}</dd></div><div><dt>Manifest hash</dt><dd>{job.manifestHash}</dd></div><div><dt>Capabilities</dt><dd>{job.coveredCapabilities.join(", ")}</dd></div><div><dt>Job</dt><dd>{job.jobId ?? "Pending assignment"}</dd></div></dl></details></li>)}</ul>}</section>; }

function GoalRail({ goal, run }: { goal: GoalSnapshot; run: GoalRunSnapshot | null | undefined }) { const proofJobs = run?.jobs.filter((job) => job.jobId !== null) ?? []; return <aside className="space-y-3"><details className="border-b border-void-800"><summary className="min-h-11 cursor-pointer py-3 font-semibold text-void-100">Spend and boundaries</summary><dl className="divide-y divide-void-800 pb-3 text-sm"><div className="flex justify-between gap-3 py-2"><dt className="text-void-500">Reserved this run</dt><dd className="text-right font-mono text-void-200">{formatUsdcAtomic(run?.totalPriceAtomic ?? "0")}</dd></div><div className="flex justify-between gap-3 py-2"><dt className="text-void-500">Per-run cap</dt><dd className="text-right font-mono text-void-200">{formatUsdcAtomic(goal.policy.perRunCapAtomic)}</dd></div><div className="flex justify-between gap-3 py-2"><dt className="text-void-500">Daily cap</dt><dd className="text-right font-mono text-void-200">{goal.policy.dailyCapAtomic ? formatUsdcAtomic(goal.policy.dailyCapAtomic) : "Not applicable"}</dd></div><div className="flex justify-between gap-3 py-2"><dt className="text-void-500">Cadence</dt><dd className="font-mono text-void-200">{goal.policy.cadenceMinutes}m</dd></div><div className="flex justify-between gap-3 py-2"><dt className="text-void-500">Policy hash</dt><dd className="max-w-[65%] break-all text-right font-mono text-void-300">{run?.policyHash ?? "Unavailable"}</dd></div></dl></details><section><div className="flex items-center justify-between gap-3"><h2 className="font-semibold text-void-100">Proof</h2><span className="font-mono text-xs text-void-500">{proofJobs.length} job{proofJobs.length === 1 ? "" : "s"}</span></div>{proofJobs.length ? <ul className="mt-2 space-y-2">{proofJobs.map((job) => <li key={job.jobId}><Link href={`/verify?jobId=${encodeURIComponent(job.jobId!)}`} className="block min-h-11 break-all py-2 font-mono text-xs text-dawg-300">{job.jobId}</Link></li>)}</ul> : <p className="mt-3 text-sm text-void-500">No job proof identifiers are available.</p>}<Link href="/verify" className="mt-2 inline-flex min-h-11 items-center text-sm font-semibold text-void-300">Open proof index</Link></section></aside>; }
