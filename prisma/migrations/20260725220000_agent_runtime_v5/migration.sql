...
CREATE TABLE public.hire_requests (
  ...
  CONSTRAINT hire_requests_result_shape_check CHECK (
    (state = 'JOB_QUEUED' AND error_code IS NULL)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    has_mcp := COALESCE(jsonb_array_length(version_row.manifest->'mcp'), 0) > 0;
    IF version_row.id IS NULL OR creator_user_id = NEW.buyer_user_id
      ...
      OR NEW.state <> (CASE WHEN has_mcp THEN 'PENDING_CONTEXT' ELSE 'JOB_QUEUED' END)
