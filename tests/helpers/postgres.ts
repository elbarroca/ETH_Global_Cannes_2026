...
let disposableTail: Promise<void> = Promise.resolve();

async function acquireDisposableSlot(): Promise<() => void> {
  const previous = disposableTail;
  let release = (): void => undefined;
  disposableTail = new Promise<void>((resolve) => {
    release = resolve;
  });
  await previous;
  return release;
...
function quotedIdentifier(value: string): string {
  if (!/^[a-z][a-z0-9_]+$/.test(value))
    throw new Error("unsafe database test identifier");
  return `"${value}"`;
}
...
function run(
  ...
  if (result.status !== 0) {
    const output = `${result.stdout ?? ""}${result.stderr ?? ""}`
      ...
      .trim();
    throw new Error(
      `database test command failed${output ? `: ${output}` : ""}`
    );
  }
}
...
export async function startDisposableDatabase(
  ...
  if (!/^[a-z0-9-]{2,32}$/.test(label))
    throw new Error("invalid database test label");
  const releaseSlot = await acquireDisposableSlot();
  try {
    ...
    if (suppliedAdminUrl) {
      ...
      const database = `alphadawg_${label.replaceAll("-", "_")}_${
        process.pid
      ...
      return {
        ...
        close: async () => {
          try {
            ...
            const cleanup = postgres(suppliedAdminUrl, {
              max: 1,
              prepare: false,
            });
            try {
    ...
    const root = await mkdtemp("/tmp/adpg-");
