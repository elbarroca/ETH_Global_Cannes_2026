...
export default function InfrastructurePage() {
  return (
    <main className="mx-auto max-w-[90rem] space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      <header className="max-w-3xl space-y-3">
        <p className="instrument-label text-dawg-400">Infrastructure</p>
        <h1 className="text-4xl font-semibold tracking-[-0.045em] text-void-100 md:text-6xl">
      ...
      <section className="rounded-xl border border-dawg-500/25 bg-dawg-500/5 p-5" aria-labelledby="verified-evidence-title">
      ...
      <section className="space-y-4" aria-labelledby="configured-identifiers-title">
        ...
        <ul className="grid gap-3 md:grid-cols-2">
          {STACK_ROWS.map((row) => (
            <li key={`${row.chain}-${row.identifier}`} className="min-w-0 rounded-xl border border-void-800 bg-black p-4">
      ...
      <details className="rounded-xl border border-void-800 bg-black">
